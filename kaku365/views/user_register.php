...
<!--登陆窗口开始-->
<div class="login">
    我已经注册，现在<a href="<?=site_url('user/login') ?>">登录</a>
</div>
<div class="register">
  <form class="register" action="<?=site_url('user/register') ?>" method="post">
    <h2>会员注册信息</h2>
    <ul>
      <li><strong>用户账号：</strong>
        <input required  id="username" name="username" type="text" class="regName" minlength="6" maxlength="20" onblur="check_username()" placeholder="登录帐号"/>
        <span id="error_username">6-20位字母，数字组合</span></li>
      <li><strong>设置密码：</strong>
        <input required id="password" name="password" type="password" class="regPassword" minlength="6" maxlength="20" placeholder="登录密码"/>
        <span>6-20位字符，建议由字母，数字和符号两种以上组合</span></li>
      <li><strong>确认密码：</strong>
        <input required id="s_password" name="s_password" type="password"  class="regPassword" maxlength="20" onblur="check_pwd()" placeholder="确认密码"/>
        <span id="error_s_password">请再次输入密码</span></li>
...
<div id="footer">
  ...
  <p> <a href="javascript:alert ('证件照片');">kaku365广告有限公司管理运营</a>| <a href="javascript:alert ('空');">企业注册号 511011000019348</a>| <a href="javascript:alert ('证件照片');">组织机构代码 56762831-7</a>| <a href="javascript:alert ('证件照片');">蜀ICP备13003878号-1</a> </p>
</div>
<!--底部信息结束-->
</body>
</html>
